...
Versioned JSONL contract for the M1 particle filter's outputs: the main
estimate stream (per `(node_id, tick)` mean + cov_diag + n_effective) and
the optional particle sidecar stream (subsampled particles + weights
subject to thinning). The two streams have separate header records and
separate readers so consumers can open one without paying for the other.
A `Protocol`-typed `ParticleStreamWriter` keeps the producer (`pf_float`)
and consumer (dashboard) decoupled from the JSONL backing — a future
binary-format swap changes the impl, not the producers or consumers.
