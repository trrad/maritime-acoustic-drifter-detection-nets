...
Regional map data structure providing bathymetry interpolation, land exclusion, shipping lane membership, current climatology queries, and onboard map fidelity reduction for the maritime particle filter's map-aided navigation.
...
The system SHALL provide a `make_onboard_map(truth_map, fidelity, seed)` function (generator-side only) that returns a deliberately imperfect `RegionalMap` derived from the truth map. The fidelity parameter (0.0–1.0) controls degradation across channels: bathymetry grid downsampling, coastline polygon simplification, and shipping lane reduction. The onboard map SHALL have a smaller hardware footprint than the truth map (measured by `hardware_footprint_bytes()`, which estimates 16-bit word count). The PF SHALL NOT call `make_onboard_map` and SHALL NOT access the truth map directly; the PF consumes the onboard map through `ScenarioReader(path).onboard_map()`.

#### Scenario: Onboard map is structurally degraded
- **WHEN** an onboard map is generated with fidelity=0.5 from a truth map
- **THEN** the onboard bathymetry grid has fewer cells than truth
- **AND** depth values differ from truth at some query points
