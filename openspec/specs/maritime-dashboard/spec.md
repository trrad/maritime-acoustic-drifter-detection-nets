...
Local HTML5 + Canvas + vanilla-JS dashboard for visualizing maritime
scenarios and PF estimates. Charter role: visual validation layer — the
tool that catches sim substance bugs numerical tolerances miss (trails
wandering through coastlines, estimate trails heading the wrong way,
LoRa link patterns that don't match the TDMA schedule). A Python CLI
reads one or more (scenario, estimates, [particles]) triples via typed
readers, inlines all data into self-contained HTML, and serves it over
a local HTTP server. Single-run mode (--scenario + --estimates)
is the pinned M1 contract; the --runs-dir multi-run selector is an
additive extension. Charter allowance for truth access is declarative:
the dashboard module is intentionally outside the PF-truth-separation
import-linter contract's source_modules.
