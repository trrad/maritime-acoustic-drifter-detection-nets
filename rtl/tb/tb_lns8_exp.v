...
module tb_lns8_exp;
    ...
    initial begin : main_block
        ...
        $display("=== EXP test (exhaustive) ===");
        fd = $fopen("vectors/exp_vectors.hex", "r");
        if (fd == 0) begin $display("ERROR: cannot open exp_vectors.hex"); $finish; end
        ...
        $fclose(fd);
        $display("EXP: %0d/%0d passed (%0d errors)", total - errors, total, errors);
